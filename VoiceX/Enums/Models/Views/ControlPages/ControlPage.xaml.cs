...
namespace VoiceX.Views.ControlPages
{
    ...
    public sealed partial class ControlPage : Page
    {
        ...
        public static Frame OpenCallPage;
        ...
        private readonly PhonePage phonePage;
        ...
        private async void ControlPage_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (PhonePage.AutoAnswerNumbers != null)
                {
                    PhonePage.AutoAnswerNumbers = JsonConvert.DeserializeObject<List<string>>(ApplicationData.Current.LocalSettings.Values["AACallList"].ToString());
                }
                else
                {
                    PhonePage.AutoAnswerNumbers = new List<string>();
                    PhonePage.AutoAnswerNumbers = JsonConvert.DeserializeObject<List<string>>(ApplicationData.Current.LocalSettings.Values["AACallList"].ToString());
                }
            }
            ...
            if (args != null && args.Request != null && args.Request.Message != null && args.Request.Message.Keys != null)
            {
                ...
                switch (select)
                {
                            ...
                            if (!Regex.IsMatch(select, "[^0-9]"))
                            {
                                try
                                {
                                    ...
                                    if (!result.Contains("success"))
                                    {
                                       ...
                                       .AddText(PhonePage.currentCall.RemoteAddress.Username, hintMaxLines: 1)
                                       .AddText("Call Not available")
                                       .AddAppLogoOverride(new Uri("ms-appx:///Assets/Icone/Person.png"), ToastGenericAppLogoCrop.Circle);
                                        builder.Show();
                                    }
                                }
                                catch
                    ...
                    case "exit": // close app
                        ...
                        break;
                    case "dialpad":
                        await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, agileCallback: () => _ = OpenWindow(typeof(PhonePage)));
                        break;
                    case "clients":
                        await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () => _ = OpenWindow(typeof(ContactsPage)));
                        break;
                    case "path": // sent file
                        if (!String.IsNullOrEmpty(args.Request.Message.FirstOrDefault().Value.ToString()))
                        {
                            ...
                            await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () => _ = OpenWindow(typeof(FaxPage)));
                        }
                        break;
                    default:
                        break;
        ...
        private async void Timer_Tick(object sender, object e)
        {
            ...
            if (difference.TotalHours > 1 && !App.MyComputer)
            {
                ...
                CoreService.Instance.ClearCoreAfterLogOut();
                timer.Stop();
