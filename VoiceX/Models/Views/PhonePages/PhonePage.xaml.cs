...
namespace VoiceX.Views.PhonePages
{
    ...
    public sealed partial class PhonePage : Page
    {
        ...
        public async void OnCallStateChanged(Core core, Call call, CallState state, string message)
        {
            switch (state)
            {
                case CallState.IncomingReceived:
                    if (currentCall == null)
                    {
                        ...
                        if (AutoAnswerNumbers.Contains(currentCall.RemoteAddress.Username))
                        {
                            ...
                            {
                                await OpenWindow(typeof(PhonePage), "Activ");
                            }
                            else
                            {
                                this.PhonePageContent.Navigate(typeof(ActivCallPage), this);
                            }
                            break;
                        }
                        if (!App.AppWindows.Contains("PhonePage"))
                        {
                            await OpenWindow(typeof(PhonePage), "Call");
                        }
                        else
                        {
                            this.PhonePageContent.Navigate(typeof(CallPage), this);
                        }
                    }
                ...
                case CallState.End:
                    ...
                    {
                        this.PhonePageContent.Navigate(typeof(DialpadCallPage), this);
                    }
                    break;
        ...
        {
            if (currentCall != null && App.AppWindows.Where(a => a == this.GetType().Name).Count() == 1)
            {
        ...
        private async void Navigate_Click(object sender, RoutedEventArgs e)
        {
            ...
            switch (Navigate.Name)
            {
                ...
                case "Fax":
                    ...
                    break;
                case "HotKeys":
                    await OpenWindow(typeof(HotKeyPage), "");
                    break;
            }
        }
