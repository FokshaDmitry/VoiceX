...
namespace VoiceX.Views
{
    ...
    public sealed partial class HistoryPage : Page
    {
        ...
        {         
            List<HistoryNotes> historyNotes = addDbContext.GetNotes();
            HistoryList.Items.Clear();
            histories.Clear();
            var groupNote = historyNotes.GroupBy(h => h.StartDialog.ToString("d.M.yyyy")).OrderByDescending(h => h.Key);
            foreach (var group in groupNote)
            {
                HistoryList.Items.Add(new HeadingContactList(group.Key.ToString()));
                foreach (var Note in group.OrderByDescending(h => h.StartDialog))
                {
                    Debug.WriteLine(Note.StatusCall.ToString());
        ...
        private void Filter_Checked(object sender, RoutedEventArgs e)
        {
            ...
            if (filter.Name == "OutCall")
            {
                ...
                foreach (var group in histories.Where(h => h.statusCall == Enums.StatusCall.Outgoing).GroupBy(h => h.dateCall.ToString("d.M.yyyy")).OrderByDescending(h => h.Key))
                {
                    HistoryList.Items.Add(new HeadingContactList(group.Key.ToString()));
            ...
            else if(filter.Name == "InCall")
            {
                ...
                CheckAll.Background = whiteLine;
                ChekIgnore.Background = whiteLine;
                foreach (var group in histories.Where(h => h.statusCall == Enums.StatusCall.Incoming).GroupBy(h => h.dateCall.ToString("d.M.yyyy")).OrderByDescending(h => h.Key))
                {
                    HistoryList.Items.Add(new HeadingContactList(group.Key.ToString()));
                    foreach (var note in group.OrderByDescending(g => g.dateCall))
                    {
            ...
            else if(filter.Name == "AllCall")
            {
                ...
                ChekIgnore.Background = whiteLine;
                foreach (var group in histories.GroupBy(h => h.dateCall.ToString("d.M.yyyy")).OrderByDescending(h => h.Key))
                {
                    HistoryList.Items.Add(new HeadingContactList(group.Key.ToString()));
                    foreach (var note in group.OrderByDescending(g => g.dateCall))
                    {
            ...
            else if (filter.Name == "IgnoreCall")
            {
                ...
                foreach (var group in histories.Where(h => h.statusCall == Enums.StatusCall.Ignore).GroupBy(h => h.dateCall.ToString("d.M.yyyy")).OrderByDescending(h => h.Key))
                {
                    HistoryList.Items.Add(new HeadingContactList(group.Key.ToString()));
                    foreach (var note in group.OrderByDescending(g => g.dateCall))
                    {
