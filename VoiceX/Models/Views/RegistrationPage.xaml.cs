    ...
    public sealed partial class RegistrationPage : Page
    {
        ...
        public async Task NetworkLogin(string pbxCode)
        {
            ...
            try
            {
                ...
                cer = new X509Certificate2(File.ReadAllBytes(certificateFile.Path), "r7Z33th35XTCfym6", X509KeyStorageFlags.Exportable);
                ...
                if (String.IsNullOrEmpty(cert.Error))
                {
                    try
                    {
                        ...
                        //var oldCrt = await addDbContext.GetCertificate();
                        ...
                        if (CertificateStores.FindAllAsync().GetAwaiter().GetResult().FirstOrDefault(c => c.FriendlyName == "app-cert") == null)
                        {
                            ...
                            // save cert in in BD
                            //await addDbContext.SaveCertificateAsunc(cert.P12);
                        }
            ...
            if (App.AccountData.ResponseCode == HttpStatusCode.OK)
            {
                try
                {
                    ...
                    rootFrame.Navigate(typeof(ControlPage), null, null);
                }
                catch (Exception ex)
                {
